            ...
            using (var ctx = TokenHelper.GetClientContextWithAccessToken(parentUrl, accessToken))
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        webinfo.UseSamePermissionsAsParentSite = !(!siteRequest.IsSiteCollectionRequest && (siteRequest.AdditionalMembers.Count > 0 || siteRequest.AdditionalAdministrators.Count > 0));
