        ...
        public void ProvisionSites(ICollection<SiteInformation> siteRequests)
        {
            ...
            foreach (var siteRequest in siteRequests)
            {
                try
                {
                    ...
                    // ****************************************************
                    if (siteRequest.IsSiteCollectionRequest || (!siteRequest.IsSiteCollectionRequest && (siteRequest.AdditionalMembers.Count > 0 || siteRequest.AdditionalAdministrators.Count > 0)))
                    {
                        Log.Info("Provisioning.Job.SiteProvisioningJob.ProvisionSites",
                            "Updating Request Access Email Address for Site Url {0}.", siteRequest.Url);
